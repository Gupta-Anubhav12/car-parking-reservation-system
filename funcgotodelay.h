...
{   string carnum,hrs,status;float dis;int i;string curdate;
    ...
    if(status=="Yes")
    {
    p<<"update vipperson set coins=coins+20 where(name,id)=('"+name+"','"+id+"');";
    }
    else
    {
p<<"update nonvipperson set coins=coins+10 where(name,id)=('"+name+"','"+id+"');";
    }
    string qp=p.str();
    int q3=mysql_query(conn,qp.c_str());
